...
export function RegisterPage() {
  ...
  const validateForm = () => {
    ...
    if (!formData.name.trim()) {
      ...
      nextErrors.name = 'Minim 3 caractere.';
    }

    const emailRegex = /^[^\s@]+@[^\s@]+\.[^\s@]+$/;
    if (!formData.email) {
      nextErrors.email = 'Email obligatoriu.';
      ...
      <div className="mx-auto grid max-w-5xl overflow-hidden rounded-[40px] bg-white/85 shadow-[0_40px_80px_rgba(15,23,42,0.18)] md:grid-cols-2">
        ...
        <div className="p-8 sm:p-12">
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <div>
              <label className="text-xs uppercase tracking-[0.3em] text-[var(--brand-slate)]/70" htmlFor="name">
                Nume complet
              </label>
              <input
                id="name"
                ...
                className={`input-field mt-2 ${errors.name ? 'border-red-300 focus:ring-red-400' : ''}`}
                placeholder="Ana Georgescu"
              />
              {errors.name && <p className="mt-1 text-sm text-red-600">{errors.name}</p>}
            </div>

            <div>
              <label className="text-xs uppercase tracking-[0.3em] text-[var(--brand-slate)]/70" htmlFor="email">
                Email
              </label>
              <input
                id="email"
                ...
                onChange={handleChange}
                className={`input-field mt-2 ${errors.email ? 'border-red-300 focus:ring-red-400' : ''}`}
            ...
            <div className="grid gap-4 sm:grid-cols-2">
              <div>
                ...
                <input
                  ...
                  onChange={handleChange}
                  className={`input-field mt-2 ${errors.password ? 'border-red-300 focus:ring-red-400' : ''}`}
                  placeholder="••••••••"
                />
              ...
              <div>
                <label className="text-xs uppercase tracking-[0.3em] text-[var(--brand-slate)]/70" htmlFor="confirmPassword">
                  Confirmă parola
                </label>
                <input
                  id="confirmPassword"
                  ...
                  onChange={handleChange}
                  className={`input-field mt-2 ${errors.confirmPassword ? 'border-red-300 focus:ring-red-400' : ''}`}
                  placeholder="••••••••"
                />
            ...
            <label className="flex items-start gap-2 text-sm text-[var(--brand-slate)]">
              <input
                type="checkbox"
                required
